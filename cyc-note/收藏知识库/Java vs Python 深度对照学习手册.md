...
这是一份为你量身定制的 **Java vs Python 运算符差异全景对照表**。作为 Java 开发者，掌握这些差异能让你写出的 Python 代码更地道，避免潜在 Bug。

---

#### 🌟 核心心法
*   **Java** 是强类型静态语言，运算符严格局限于数值和布尔逻辑，结果多为布尔值。
*   **Python** 是动态类型语言，运算符具有更强的“泛化”能力（如支持链式比较、逻辑运算返回原值），且强调可读性（使用单词而非符号）。

---

#### 一、 算术运算符 (Arithmetic)

| 运算 | Java 写法 | Python 写法 | 核心差异说明 |
| :--- | :--- | :--- | :--- |
| **除法** | `5 / 2` → `2` | `5 / 2` → `2.5` | **最易踩坑！** Java 整数相除截断小数；Python 无论是否整除，**永远返回浮点数**。 |
| **整除(取整)** | `5 / 2` → `2` | `5 // 2` → `2` | Python 用 `//` 明确表示整除。**注意：负数整除时，Python 向负无穷取整**：`-5 // 2` → `-3` (Java是 `-2`)。 |
| **幂运算** | `Math.pow(2, 3)` | `2 ** 3` | Python 原生支持 `**` 运算符，比调用方法更简洁高效。 |
| **取模(求余)** | `5 % 2` → `1` | `5 % 2` → `1` | 正数表现一致。**负数取模规则不同**：Python 结果的符号与除数一致，Java 与被除数一致。 |

**示例：除法与整除的陷阱**
```java
// Java
System.out.println(5 / 2);       // 2 (截断)
System.out.println(-5 / 2);      // -2 (向零取整)
```
```python
# Python
print(5 / 2)       # 2.5 (永远返回浮点数)
print(5 // 2)      # 2   (整除)
print(-5 // 2)     # -3  (向下取整，地板除！这与Java结果不同！)
```

**示例：取模的陷阱**
```java
// Java
System.out.println(-5 % 2);      // -1 (符号跟随被除数 -5)
```
```python
# Python
print(-5 % 2)      # 1  (符号跟随除数 2，因为 -5 = -3 * 2 + 1)
```
...
| 运算 | Java 写法 | Python 写法 | 核心差异说明 |
| :--- | :--- | :--- | :--- |
| **判等** | `a == b` | `a == b` | Python 调用 `__eq__` 比较值。Java 比较基本类型值，对象比较引用。 |
| **判不等** | `a != b` | `a != b` | 一致。 |
| **身份比较** | `a == b` (对象) | `a is b` | **重点！** Python 用 `is` 判断是否为同一个内存对象（相当于Java的对象 `==`）。常用于 `is None`。 |
| **链式比较** | `a > 0 && a < 10` | `0 < a < 10` | **Python 语法糖**。更接近数学表达，且性能更高（a只计算一次）。 |
...
# 甚至可以连缀不同运算符
if 0 < x < 100 == y:  # 等价于 (0 < x) and (x < 100) and (100 == y)
    pass
```
...
| 运算 | Java 写法 | Python 写法 | 核心差异说明 |
| :--- | :--- | :--- | :--- |
| **逻辑与** | `a && b` | `a and b` | Python 使用英文单词，可读性更强。 |
| **逻辑或** | `a \|\| b` | `a or b` | 同上。 |
| **逻辑非** | `!a` | `not a` | 同上。 |
| **返回值** | 返回 `boolean` | **返回决定结果的操作数** | **核心差异！** Java 返回 true/false，Python 返回最后一个被评估的值本身。 |
| **隐式布尔** | 必须是布尔表达式 | 任何对象均可判真伪 | Python 中 `0`, `None`, `""`, `[]`, `{}` 均为假。 |

**示例：返回原值的妙用（Pythonic 风格）**
```java
// Java 设置默认值
String name = (input != null && !input.isEmpty()) ? input : "Guest";
```
```python
# Python 设置默认值 (极简)
name = input or "Guest"
# 如果 input 为 "" 或 None，or 需要看后面，返回 "Guest"

# and 的用法：安全取属性
result = obj and obj.property  # 如果 obj 为 None，直接返回 None，不报错
```
...
| 运算 | Java 写法 | Python 写法 | 核心差异说明 |
| :--- | :--- | :--- | :--- |
| **自增/自减** | `i++`, `i--` | **不支持！** | Python 只能用 `i += 1` 或 `i -= 1`。 |
| **复合赋值** | `+=`, `-=` 等 | `+=`, `-=` 等 | 一致。但 Python 新增了整除赋值 `//=` 和 幂赋值 `**=`。 |

**示例：为什么 Python 没有 `++`？**
因为在 Python 中，数字是不可变对象，`i++` 语义上是在原对象上修改，这违背了 Python 的设计原则。`i += 1` 实际上是创建了一个新的数字对象并重新绑定引用。
...
| **按位与/或/异或/取反**| `&`, `\|`, `^`, `~`| `&`, `\|`, `^`, `~` | 符号一致。 |
| **左移** | `<<` | `<<` | 一致。 |
| **右移** | `>>` (带符号), `>>>` (无符号) | `>>` (带符号) | **Python 没有无符号右移 `>>>`**。因为 Python 整数无限精度，没有固定的位宽概念。 |

**示例：模拟 Python 的无符号右移**
```python
# Java: int x = -1 >>> 8;
# Python 需要手动掩码来模拟 32 位无符号右移：
...
这些是 Java 中完全不存在，但极常用的 Python 特性：

| 运算符 | 名称 | 说明 | 示例 |
| :--- | :--- | :--- | :--- |
| **`in` / `not in`** | 成员运算符 | 判断元素是否在序列中（替代 Java 的 `contains()`） | `"a" in ["a", "b"]` → `True` |
| **`is` / `is not`** | 身份运算符 | 判断内存地址是否相同（替代 Java 的对象 `==`） | `a is None` |
| **`@`** | 矩阵乘法 | 用于 NumPy 等库的矩阵运算 | `A @ B` |
| **`:=`** | 海象运算符 | 3.8 新增，在表达式中赋值并返回值 | `if (n := len(a)) > 10: ...` |
...
# Python 统一用 in
if "apple" in my_list: ...
if "key" in my_dict: ...   # 字典判断的是 key
if "sub" in my_string: ... # 字符串判断子串
```
...
| :--- | :--- | :--- |
| `obj instanceof String` | `isinstance(obj, str)` | Python 推荐用 `isinstance` 而不是 `type(obj) == str`，因为前者支持继承判断。 |

---

#### 💡 总结建议

1.  **写 Python 代码时，忘掉 `++`、`&&`、`||` 和 `!`**，IDE 会报错，但思维方式要主动切换。
2.  **永远不要用 `==` 判断 `None`**，用 `is None`。
3.  **拥抱 `in` 和 `or` 的语法糖**，少写 `contains()` 和冗长的三元表达式。
4.  **小心除法 `/` 和 整除 `//`**，特别是在涉及负数运算时，Python 的行为与 Java 截断取整的逻辑不同。
