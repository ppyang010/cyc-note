---
id: "632484805"
title: "Java中如何获得A<T>泛型中T的运行时类型及原理探究"
author: "千问云"
type: zhihu-article
source: "https://zhuanlan.zhihu.com/p/632484805"
created: "2023-05-29 13:02"
updated: "2023-05-29 13:02"
collected: "2023-05-29 13:02"
downloaded: "2026-08-16"
---
**完整文章内容可点击下方链接查看：**

[Java中如何获得A&lt;T&gt;泛型中T的运行时类型及原理探究-阿里云开发者社区](https://link.zhihu.com/?target=https%3A//developer.aliyun.com/article/1226646%3Futm_content%3Dg_1000372882)
...
泛型是Java1.5引入的特性，主要是为了解决类型检查（Type Checking）问题 [1](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/tutorial/java/generics/why.html)，为我们编写通用代码（尤其是库）时提供强有力的类型约束，而不用担心1.5之前满天乱飘的cast导致heap pollution[13](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/tutorial/java/generics/nonReifiableVarargsType.html%23heap_pollution)。

为什么要引入泛型，泛型设计的思路，可以参考Gilad Bracha在1998年OOPLSA会议上的论文 [2](https://link.zhihu.com/?target=https%3A//homepages.inf.ed.ac.uk/wadler/gj/Documents/gj-oopsla.pdf)。后来形成了JSR14：Adding Generics to the Java Programming Language

add generics in java[14](https://link.zhihu.com/?target=https%3A//jcp.org/aboutJava/communityprocess/review/jsr014/index.html)，并最终在1.5进入JDK。

泛型的形式化定义参考JLS[3](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/specs/jls/se8/html/jls-8.html%23jls-8.1.2),[4](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/specs/jls/se8/html/jls-8.html%23jls-8.4.4),[5](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/specs/jls/se8/html/jls-8.html%23jls-8.8.4),[6](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/specs/jls/se8/html/jls-9.html%23jls-9.1.2)
...
因为1.5引入了泛型，所以反射也针对新概念，做了相应的扩展[7](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/1.5.0/docs/guide/reflection/enhancements.html)。
...
其中我们需要打最多交道的，就是ParameterizedType[10](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/specs/jls/se8/html/jls-4.html%23jls-4.5)
...
同时伴随着泛型落地，反射API里增加了一系列名称中含有Generic的方法和类[8](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/8/docs/api/java/lang/reflect/class-use/Type.html)，这些都是之后我们获得泛型运行时类型的基础。

  

## 类型擦除（Type Erasure）

虽然Java引入了泛型这个概念，但为了保持向前兼容（JVM层面的兼容，不需要改动Bytecode和JVM设计），和编译性能（相比C++的Template会以模板参数生成新的类型），Java引入了类型擦除[9](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/specs/jls/se8/html/jls-4.html%23jls-4.6)作为泛型的实现方案。通过类型擦除，Java不需要在虚拟机实现上做任何修改，同时也不会为ParameterizedType创建新的类。
...
其实它利用了JSR14[14](https://link.zhihu.com/?target=https%3A//jcp.org/aboutJava/communityprocess/review/jsr014/index.html)中，对类中的泛型信息，保存到类签名（Signature）的一个技巧。
...
其中的attributes数组里，就是JSR14里提到的，泛型信息的保存所在。JVMS指出[11](https://link.zhihu.com/?target=https%3A//docs.oracle.com/javase/specs/jvms/se8/html/jvms-4.html%23jvms-4.7.9.1)
...
根据《The Hitchhiker's Guide to javac》[15](https://link.zhihu.com/?target=https%3A//openjdk.org/groups/compiler/doc/hhgtjavac/index.html)，JavaCompiler是javac的driver。所以我们可以通过研究JavaCompiler的实现，来研究javac编译文件的过程（通过strace javac的编译过程，可以看到调用里含有JavaCompiler。基本可以得到相同的结论）。
...
![](images/482_002.png)![](images/482_003.png)
...
其中getGenericSignature0对应JNINativeMethod这个struct的一个对象。{"getGenericSignature0", "()" STR, (void \*)&JVM\_GetClassSignature}
...
可见getGenericSignature0对应的jvm实现是JVM\_GetClassSignature，一个函数指针。

这个函数的实现在./hotspot/src/share/vm/prims/jvm.cpp，包裹在JVM\_ENTRY宏里。
...
可以看到，最终getGenericSignature0是从InstanceKlass::cast(k)->generic\_signature方法获得。而这个方法使用\_generic\_signature\_index这个序号从ClassFile的Symbol数组里获取相关数据。与javac编译过程的源码和JVMS标准是符合的。
...
[阿里云开发者社区-云计算社区-阿里云](https://link.zhihu.com/?target=https%3A//developer.aliyun.com/%3Futm_content%3Dg_1000371672)