---
id: "2064610816037540542"
title: "你的 Agent 架构，为什么半年后就过期了？"
author: "数字工程师教练"
type: zhihu-article
source: "https://zhuanlan.zhihu.com/p/2064610816037540542"
created: "2026-07-26 07:56"
updated: "2026-07-26 23:47"
collected: "2026-07-26 07:56"
...
> 你的 Agent 架构，半衰期只有六个月。

演讲者是 Inngest 联合创始人兼 CTO Dan Farrelly。

这句话并不是说所有 Agent 项目半年后都会报废，而是在提醒我们：今天看起来很先进的模型、Prompt、工具协议和 Agent 编排方式，半年后很可能已经不是主流。如果系统架构与这些快速变化的部分深度绑定，技术栈每升级一次，团队就要跟着重写一次。
...
-   RAG、ReAct、Prompt Chaining；
-   Function Calling、MCP、CLI；
-   单 Agent、多 Agent、Sub-agent；
-   Coding Agent、Background Agent、Agent Factory；
-   云端 Sandbox、本地执行环境、浏览器自动化。

这些方案都可能有价值，问题在于它们变化得太快。

新模型出现后，旧 Prompt 可能失效；新的工具协议出现后，原有调用链可能要重构；当任务从一次对话变成运行数小时的后台流程时，原先围绕“请求—响应”设计的系统又会迅速触顶。
...
Dan 把 Agent 系统分成三个概念层：计算层、上下文层和执行层。

### 1\. 计算层：负责思考

计算层的核心是模型。它负责推理、规划、生成和判断。

但模型也是替换最频繁的组件之一。团队可能因为能力、成本、速度或上下文窗口，随时切换到新的模型，甚至在一次任务中动态路由多个模型。
...
执行层负责的是那些不够“性感”、却决定 Agent 能否进入生产环境的能力：
...
-   把恢复逻辑写进 Sandbox，会让业务逻辑和基础设施紧密耦合。
...
而且真实结果经常延迟出现。PR 可能几天后才被合并，研究报告可能一周后才被引用。评估系统必须能把这些后续事件重新关联到最初那次 Agent Run。
...
Dan 是 Inngest 的联合创始人，而 Inngest 本身提供持久执行、队列、流量控制和可观测等能力。所以这场演讲天然会强调执行层的重要性，也与其产品定位一致。

但这并不意味着观点没有价值。我们需要区分：

1.  **普遍的架构原则**：把可靠执行与快速变化的模型、Prompt 和工具解耦；
2.  **具体的产品选择**：使用 Inngest、自建工作流引擎，还是采用其他持久执行平台。
...
这场演讲最打动我的，不是它推荐了哪个框架，而是它提供了一种判断架构边界的方法：
