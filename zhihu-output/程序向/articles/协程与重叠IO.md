...
author: "microcai"
type: zhihu-article
source: "https://zhuanlan.zhihu.com/p/7424498247"
created: "2024-11-18 08:17"
updated: "2024-11-18 08:17"
collected: "2024-11-18 08:17"
...
由于 c++ 协程的设计缺陷，c++ 协程只有在被 co\_await 的时候，才真正开始执行。而不是创建的时候执行。

> 为防止杠精说，c++ 协程通过设置 initual\_suspend 也能实现 创建的时候开始执行。问题是，作为一个 协程IO库，你的操作 系统底层是使用的“事件通知”+回调函数实现的。因此必须获得当前协程的睡眠句柄，才能进行在回调处理里唤醒协程这样的操作。 而当前协程的句柄，就必须得通过 co\_await 调用才能拿到。也就是说，如果在没拿到协程句柄的情况下，就发起 IO 操作， 就要实现复杂的 “稍后获得协程句柄后提交给已经挂入系统IO回调列队的回调函数” 逻辑。这个逻辑非常难以编写。并且有潜在的 并发和数据竞争问题。
